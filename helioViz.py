...
stationaryMirrorHeight = 11.0
stationaryMirrorWidth = 9.0
distanceStationaryMirrorToRotatedMirror = 32

#Used to set the heliostat locations and ray tracing
...
#Used to set the power tower location and ray tracing
powerTowerDimension = 12
powerTowerYoffset = -112
powerTowerZoffset = 63

#The following create the objects in the visualization

#These are the local - green, red, blue axis's
#The entire field is based around these local coodinates
#Green = X axis
#Red = Y axis
...
#Output: Change in x and y to move the heliostat radially around the power tower
...
class rotatedMirror:
    ...
    #h is an array with the corner coordinates of the helistat around the origin.
    #To make multiple helistats, it is transposed to different coordinates using this array
    #Enclosure defines the coordinates of the enclosure around the heliostat mirrors
    def rotatedMirror(self):
        # This code sux
        # Essentially, it moves the corners of the heliostat to so the normal of the
    ...
    # Changes the orientation of the Heliostats to match the trackingVX output
...
#Helper method to rotate the stationary mirrors around the x and y axis
#Returns the coordinates of the corners of the stationary mirror after the rotations
...
#Ray class is a linked list of vectors that represent light rays bouncing off mirrors.
#Each ray that is created starts off not being reflected, but if it is reflected,
#then it can have a next vector that represents the light after it is reflected.
class Ray:
...
#Displays a 'sun' at the inputted vector location
def sun(sunRotation):
...
# Calls the tracking algorithm's main and returns the array of absolute visual x
...
#Returns a reflected vector of the incomingVector hitting a surface with a normal vector
...
#Returns the x,y,z coordinates for the point where the sun vector hits the rotated heliostat.
#fractionHeight, Width are the % up the Y axis and X axis that the points need to start.
#Uses the normal, a corner of the heliostat to define a plane, and the fraction with
#height and width to find the coordinates that the point is located.
...
#Returns a boolean of whether or not the incoming ray hits the stationary mirror
...
#Solve for t
#Plug in t for x,y,z
#return Find distance between x(t),y(t),z(t) and starting Location
#Return the point where the light hits the stationary mirror - this is the starting location
#of the next reflection
...
#Draws all the rays that are connected in the linked list
...
def optimizeStationaryMirrors(rayArray):
    sumReflected = 0
    for row in range(len(rayArray)):
        for col in range(len(rayArray[row])):
            if rayArray[row][col].getNextReflected().reflected == True:
                sumReflected = sumReflected + 1
    sumReflected = float(sumReflected)
    return sumReflected / len(rayArray)**2
...
class Game(object):
    ...
    def __init__(self):
        ...
        #MUST BE a integer, 2^2, 3^2.. etc is the actual number of rays in a square
        self.numberOfRays = 5
    ...
    #Helper
    # Instead of having to update these vectors in every input case, this reduces # lines
    ...
    #Creates the light coming from the sun that bounces off the mirrors
    #The light rays are stored in an array, where each index is a linked list
    #which contains all the light rays connected together
    ...
    def main(self):
        ...
        #Clock- specify how long to wait between screens (# screens/sec)
        #clock = pygame.time.Clock()

        #Create the window that displays the game, and set initial parameters for the screen
        initialization()

        # workbook = xlsxwriter.Workbook('optimizeStationaryMirrors.xlsx')
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                if(event.type == pygame.KEYDOWN and event.key == pygame.K_p and trackingOn == False):
                    ...
                    trackingOn = False
                # If g is pressed, switch boolean for groundOn
                if(event.type == pygame.KEYDOWN and event.key == pygame.K_g and groundOn == False):
                    groundOn = True
                elif(event.type == pygame.KEYDOWN and event.key == pygame.K_g and groundOn == True):
                    groundOn = False
                #If h is pressed, switch boolean for skyOn
                if(event.type == pygame.KEYDOWN and event.key == pygame.K_h and skyOn == False):
                    skyOn = True
                elif(event.type == pygame.KEYDOWN and event.key == pygame.K_h and skyOn == True):
                    skyOn = False
                ...
                #Print after every user event
                #if((not trackingOn) and event.type == pygame.KEYDOWN or (event.type == pygame.MOUSEMOTION and (pygame.mouse.get_pressed())[0])):
            #        print("X absolute coordinates:  "+str(self.xAbsoluteToLocal))
        #            print("Y absolute coordinates:  "+str(self.yAbsoluteToLocal)+"\n")
            ...
            if(trackingOn):
                ...
                if(rayOn):
                    ...
                    # worksheet.write("A"+str(currentExcelLine), currentTime)
                    # worksheet.write("B"+str(currentExcelLine), optimizeStationaryMirrors(self.rayArray2D_1))
